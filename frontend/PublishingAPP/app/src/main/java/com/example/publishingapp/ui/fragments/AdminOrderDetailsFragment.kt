...
class AdminOrderDetailsFragment : Fragment(R.layout.fragment_order_details_admin) {

    companion object {
        private const val ARG_ORDER_ID = "order_id"

        fun newInstance(orderId: Long): AdminOrderDetailsFragment {
            return AdminOrderDetailsFragment().apply {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnChangeStatus = view.findViewById(R.id.btnChangeStatus) ?: return
        ...
        btnChangeStatus.setOnClickListener {
            order?.let { order ->
                (parentFragment as? AdminOrdersFragment)?.showStatusChangeDialog(order)
            }
        }
    ...
    private fun updateStatusButton(status: String) {
        ...
        btnChangeStatus.visibility = if (isEditable) View.VISIBLE else View.GONE

        if (isEditable) {
            btnChangeStatus.text = "Изменить статус ($status)"
        }
    }
